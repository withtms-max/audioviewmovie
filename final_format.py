...
# 5. Enrichment Logic for Keywords (for API movies only or as fallback)
def get_cast_and_enrich_keywords(title, orig_genres, orig_keywords, director, row_has_api):
    # If it was manual data, we might want to keep user's keywords but can append cast if accessible
    # To keep it simple and respect user's manual input, we only auto-enrich if it came from API
    # or if keywords are missing.
    
    # Check if we should fetch cast (mostly for API movies)
    cast_names = []
    if row_has_api:
        try:
            ...
            if results:
                ...
                cast_names = [c['name'] for c in cred_res.json().get('credits', {}).get('cast', [])[:5]]
        except:
            pass
    ...
    if cast_names:
        keywords_parts.extend(cast_names)
    if orig_genres:
        keywords_parts.extend([g.strip() for g in str(orig_genres).split(",")])
    if director:
        keywords_parts.append(str(director))
    if orig_keywords and str(orig_keywords) != "nan":
        keywords_parts.extend([k.strip() for k in str(orig_keywords).split(",")])
            
    seen = set()
    final_list = []
    for k in keywords_parts:
        if k and str(k).strip() and k not in seen:
...
for idx, row_data in df_sorted.iterrows():
    ...
    orig_keywords = row_data.get('검색 키워드', '')
    director = row_data.get('감독', '')
    
    print(f"[{idx+1}/{len(df_sorted)}] '{title}' 처리 중...", flush=True)
    
    # Determine if it's from API or Manual for the cast fetch (Optional optimization)
    # For now, if has_info is true, we attempt enrichment
    final_keywords = get_cast_and_enrich_keywords(title, orig_genres, orig_keywords, director, row_data['has_info'])
    ...
    if row_data['has_info'] and poster_url.startswith("http"):
        try:
            ...
            if img_res.status_code == 200:
                img_path = f"temp_posters/final_merged_{row_num}.jpg"
